...
export default function ProductCompareTray({ products = [], onClear, onView }) {
  ...
  return (
    <div className="border p-3 font-mono space-y-2 overflow-hidden" style={{ borderColor: '#5C4424', background: '#100D09' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        {products.map((p) => {
          const standard = roundPrice(p.price_auec || 0);
          const member = roundPrice(standard * (100 - REDSCAR_DISCOUNT_PERCENT) / 100);
          return (
            <button key={p.id} onClick={() => onView?.(p)} className="text-left border p-2.5 md:p-2 hover:brightness-110 min-w-0" style={{ borderColor: '#3A2F20', background: '#0C0A07' }}>
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-1 mt-2 text-[9px]">
                <span style={{ color: '#6B6155' }}>STANDARD</span><span style={{ color: '#E0A22E' }}>{standard.toLocaleString()}</span>
