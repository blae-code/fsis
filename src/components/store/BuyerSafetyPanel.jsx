...
export default function BuyerSafetyPanel() {
  return (
    <div className="border p-2.5 font-mono" style={{ borderColor: '#5C4424', background: 'rgba(212,146,11,0.05)' }}>
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1 mt-1">
            {RULES.map((rule) => <span key={rule} className="text-[8px]" style={{ color: '#A89C8A' }}>• {rule}</span>)}
          </div>
        </div>
      </div>
    </div>
