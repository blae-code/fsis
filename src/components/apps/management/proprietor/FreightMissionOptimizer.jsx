...
import FreightMissionPasteImporter from './FreightMissionPasteImporter';
import FreightMissionRunCard from './FreightMissionRunCard';
import FreightMissionTimeline from './FreightMissionTimeline';
...
export default function FreightMissionOptimizer() {
  ...
  return <div className="border p-4 font-mono space-y-3" style={{ borderColor: '#3A2F20', background: '#0A0806' }}><div className="flex justify-between gap-3"><div><p className="text-[10px] tracking-[0.2em]" style={{ color: '#8A8F45' }}>MISSION CHAIN OPTIMIZER</p><h3 className="text-sm font-bold" style={{ color: '#F2EADC' }}>Route-aware chain planner</h3></div><button onClick={saveChain} disabled={!chosen.length || update.isPending} className="border px-3 text-[10px] tracking-[0.14em]" style={{ borderColor: '#8A6430', color: '#E0A22E' }}><Wand2 className="w-3 h-3 inline mr-1" />SAVE CHAIN</button></div><FreightMissionPasteImporter onImported={(created) => { const rows = Array.isArray(created) ? created : []; setSelected(rows.map((m) => m.id).filter(Boolean).slice(0, 10)); refresh(); }} /><FreightMissionForm form={form} setForm={setForm} onSubmit={submit} /><FreightMissionRunCard missions={chosen} capacity={capacity} setCapacity={setCapacity} /><FreightMissionTimeline missions={chosen} /><div className="grid lg:grid-cols-2 gap-3"><FreightMissionList missions={active} selected={selected} setSelected={setSelected} /><FreightOptimizedChain missions={chosen} onStatus={setMissionStatus} /></div><p className="text-[9px]" style={{ color: '#8A7E6C' }}>The optimizer now links profitable contracts by reward-per-SCU, priority, lower risk, and route continuity from the last destination.</p></div>;
}