...
/** Chevron plate — a process line read left to right, stamped rather than tabbed. */
...
/**
 * The shared console rail: an ordered run of stages with what is owing on each.
 * Used by every deck so a comrade learns the chrome once.
 */
export default function DeckChevronRail({ railId, items, active, onSelect, counts = {}, spine }) {
  const activeIndex = items.findIndex((s) => s.id === active);
  return (
    <div className="shrink-0">
      {spine && (
        <div className="flex items-center gap-2 mb-1">
          <span className="text-[7px] tracking-[0.34em]" style={{ color: '#4A4136' }}>{spine}</span>
          <div className="flex-1 h-px" style={{ background: 'linear-gradient(90deg,#2A2118,transparent)' }} />
          <span className="text-[7px] tracking-[0.24em] tabular-nums" style={{ color: '#4A4136' }}>
